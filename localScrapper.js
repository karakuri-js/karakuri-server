...
const isVideoExtension = extension =>
  ["3gp", "mp4", "flv", "avi", "webm", "mkv", "wmv", "mpg"].indexOf(
    extension.toLowerCase()
  ) !== -1;

function getFileInfos(fileName, dirPath, stat) {
  const lastIndexOfSlash = dirPath.lastIndexOf("/");
  const dirName = lastIndexOfSlash !== -1
    ? dirPath.substr(lastIndexOfSlash + 1)
    : ".";
  const fileNamePatterns = fileName.match(
    /^(.+) - ([A-Z0-9 ]+) - (.+)\.(.{2,4})$/
  ) || [];
  ...
  new Promise((resolve, reject) => {
    getDirectoryContents("./karaoke", (err, contents) => {
      ...
      const allContents = contents
        ...
        .map((element, id) =>
          Object.assign(omit(element, ["isDir", "isFile", "isVideo"]), { id })
        );
      resolve(allContents);
    });
  });
